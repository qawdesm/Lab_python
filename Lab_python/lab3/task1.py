...
class Client:
    ...
    def get_total_balance(self):
        ...
        for account in self.accounts.values():
            rate = exchange_rates.get(account.currency, 1.0)
...
class BankSystem:
    ...
    def deposit(self):
        ...
        try:
            account = self.bank.clients[self.current_client].get_account(currency)
            if not account:
                raise ValueError(f"Счет в валюте {currency} не найден")
    ...
    def transfer(self):
        ...
        try:
            from_account = self.bank.clients[self.current_client].get_account(from_currency)
            if not from_account:
                raise ValueError(f"Счет в валюте {from_currency} не найден")
    ...
    def show_accounts(self):
        ...
        print(f"ОБЩИЙ БАЛАНС: {total_balance:.2f} RUB")
